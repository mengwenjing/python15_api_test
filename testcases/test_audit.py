...
class TestAudit(unittest.TestCase):
    ...
    logger.info('开始执行TestAudit测试类')

    testdata = DoExcel(case_file,'audit').read_data()
    ...
    def testInvest(self,case):
        ...
        try:
            ...
            DoExcel(case_file,'audit').write_data(case.case_id+1,res.text,'failed')
            logger.error("断言失败，错误信息是：{}".format(e))
        else:
            DoExcel(case_file,'audit').write_data(case.case_id+1,res.text,'pass')
            logger.info('用例执行成功')
